...
class SiFT_MTP:
    ...
    def receive_msg(self):
        ...
        # self.received_sequence_num = int.from_bytes(parsed_msg_hdr['sqn'], byteorder='big')
        # print(self.received_sequence_num)
        # print(self.send_sequence_num)
        # if self.received_sequence_num <= self.send_sequence_num:
        ...
        if parsed_msg_hdr['typ'] == self.type_login_req:
            ...
            #     decrypted_payload = aesgcm.decrypt(nonce, msg_body, None)
            #     print("Decryption successful. Payload decrypted.")
            ...
            decrypted_payload = cipher.decrypt_and_verify(msg_body, msg_mac)
            print("Decryption successful. Payload decrypted.")
        
        elif parsed_msg_hdr['typ'] == self.type_login_res:
            print("LOGIN RESPONSE HAD BEEN INITIATED")
            print("---------------------------------")
            cur_key = self.tk
            # Time to decrypt message
            nonce = parsed_msg_hdr['sqn'] + parsed_msg_hdr['rnd']
            ...
            # print(f"Decrypted Temporary Key (tk): {cur_key.hex()}")
            ...
            # try:
            #     # Decrypt the payload and verify the MAC (epd includes the encrypted payload and MAC)
            #     decrypted_payload = aesgcm.decrypt(nonce, msg_body, None)
            #     print("Decryption successful. Payload decrypted.")
            #     print(f"Decrypted Payload: {decrypted_payload.hex()}")
        ...
        if parsed_msg_hdr['typ'] == self.type_login_req:
            ...
            print("this is the code that runs when the other mtps are sent.....")
    ...
    def send_msg(self, msg_type, msg_payload):
        ...
        if self.DEBUG:
            ...
            print(f"Header message_type (Protocol version): {msg_type.hex()}")
        ...
        if msg_type == self.type_login_req:
            ...
            key = self.tk
            print("this is the tk that was generated: " + key.hex())
